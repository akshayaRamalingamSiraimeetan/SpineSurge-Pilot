/**
 * Pedicle landmarking + screw-sizing logic (probes a CT volume for bone boundaries).
 *
 * PORTED VERBATIM from the old repo (src/renderer/features/measurements/planning/PedicleLogic.ts) as
 * part of Step 8b. Behavior unchanged. The pure helpers (`calculatePedicleAxis`,
 * `calculateScrewDistance`, `formatCoordinateData`) are parity-locked in test/golden; the
 * volume-probing functions (`probeBoneBoundary`, `estimateScrewDimensions`, `calculateBoneContact`)
 * read a live Cornerstone volume and are parity-proven against a synthetic volume in
 * PedicleLogic.probe.test.ts (old==new). Do not change results without a divergence entry.
 */
import { cache } from '@cornerstonejs/core';
import { add, scale, sub, normalize, mag, type Vec3 } from './SurgicalGeometry';
