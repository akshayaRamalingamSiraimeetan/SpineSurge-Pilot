/**
 * Custom 3D Volumetric Eraser — erases all voxels within a 2D region through the entire volume depth.
 *
 * PORTED from the old repo (src/renderer/features/dicom/volumeEraser.ts) as part of Step 8b. Behavior
 * unchanged. Port-conformance edit only: dropped the unused `Enums as ToolsEnums` import (imported but
 * never referenced in the old source). `getViewportOrientation` (pure string logic) is parity-locked
 * in test/golden; `erase3DRegion` mutates a live Cornerstone volume and is exercised in the pipeline.
 */
import { cache } from '@cornerstonejs/core';

export function erase3DRegion(
    segmentationId: string,
    _viewportId: string, // unused in the old source; kept positional, prefixed for the strict tsconfig
