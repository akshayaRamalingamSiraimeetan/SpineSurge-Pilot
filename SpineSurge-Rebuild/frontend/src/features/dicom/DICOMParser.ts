/**
 * Browser-side DICOM file parser (dicom-parser) → normalized slice array.
 *
 * PORTED from the old repo (src/renderer/features/dicom/DICOMParser.ts) as part of Step 8b. Behavior
 * unchanged. Port-conformance edits only: the `DICOMResource` domain type is defined inline (the old
 * shared lib/store/types.ts is not ported wholesale). The pure intensity transforms `getPixelValue`
 * (HU rescale) and `mapToGrayscale` (windowing) are parity-locked in test/golden; `parseDicomFiles`
 * needs real DICOM bytes and is exercised in the live/visual pipeline (LOGIC_PORT_PLAN §2).
 */
import dicomParser from 'dicom-parser';

/** Minimal resource shape consumed by the parser (file-like). */
