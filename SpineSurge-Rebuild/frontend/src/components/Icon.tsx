/* Line icon set (24x24, stroke=currentColor) — ported verbatim from the Claude Design build
   (frontend/SS/app/icons.jsx). Multi-segment paths are joined with `__`. */
...
const ICON_PATHS: Record<string, string> = {
  ...
  settings: 'M12 9.2a2.8 2.8 0 1 0 0 5.6 2.8 2.8 0 0 0 0-5.6Z__M19.2 12c0-.5-.05-.95-.13-1.4l2-1.55-2-3.46-2.35.95a7 7 0 0 0-2.42-1.4L13.9 2.6h-3.8l-.4 2.54a7 7 0 0 0-2.42 1.4L4.93 5.6l-2 3.46 2 1.55c-.08.45-.13.9-.13 1.4s.05.95.13 1.4l-2 1.55 2 3.46 2.35-.95a7 7 0 0 0 2.42 1.4l.4 2.54h3.8l.4-2.54a7 7 0 0 0 2.42-1.4l2.35.95 2-3.46-2-1.55c.08-.45.13-.9.13-1.4Z',
...
export function Icon({ name, size = 20, style, strokeWidth = 1.7, className }: IconProps) {
...
export function Wordmark({ size = 17 }: { size?: number }) {
  return (
    <span style={{ fontWeight: 800, fontSize: size, letterSpacing: '-.02em', display: 'inline-flex' }}>
      <span style={{ color: 'var(--text)' }}>Spine</span>
      <span style={{ color: 'var(--accent)' }}>Surge</span>
    </span>
  );
}
