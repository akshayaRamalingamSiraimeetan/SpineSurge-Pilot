...
import { splitPolygonByLine, getPolygonCenter, isPointInPolygon, type Point } from './GeometryUtils';
...
/* applyOperation parameter typing (Step 7c refinement — MASTER §8.4).
 * The old signature was `applyOperation(operationType: string, params: any)`. Below is the
 * discriminated map of operation → its exact parameters, so call sites are type-checked and the
 * `any` is gone. Behaviour is unchanged: the operation handlers read the same fields they always
 * did; each handler casts the incoming params to its own member of this map. */
export type OperationType =
  | 'ROTATE' | 'CUT' | 'DELETE_FRAGMENT' | 'MOVE' | 'FREEHAND_CUT'
  | 'ADD_MEASUREMENT' | 'UPDATE_MEASUREMENT' | 'DELETE_MEASUREMENT'
  | 'WEDGE_OSTEOTOMY' | 'REPLACE_FRAGMENTS' | 'UPDATE_FRAGMENT'
  | 'ADD_IMPLANT' | 'UPDATE_IMPLANT' | 'DELETE_IMPLANT' | 'MOVE_IMPLANT';

/** Fields common to every operation. */
interface BaseOpParams { skipHistory?: boolean; }

export interface OperationParamsMap {
    ROTATE: BaseOpParams & { fragmentId: string; angleDelta: number; center?: Point };
    CUT: BaseOpParams & { startPoint: Point; endPoint: Point; fragmentId?: string };
    DELETE_FRAGMENT: BaseOpParams & { fragmentId: string };
    MOVE: BaseOpParams & { fragmentId: string; deltaX: number; deltaY: number };
    FREEHAND_CUT: BaseOpParams & { points: Point[] };
    ADD_MEASUREMENT: BaseOpParams & {
        toolKey: string; fragmentId?: string | null; points: Point[]; result?: unknown; measurement?: unknown;
    };
    UPDATE_MEASUREMENT: BaseOpParams & {
        id: string; points?: Point[]; measurement?: Record<string, unknown>; result?: unknown;
    };
    DELETE_MEASUREMENT: BaseOpParams & { id: string };
    WEDGE_OSTEOTOMY: BaseOpParams & { points: Point[]; type: string; id?: string };
    REPLACE_FRAGMENTS: BaseOpParams & { oldFragmentId: string; newFragments: Fragment[] };
    UPDATE_FRAGMENT: BaseOpParams & {
        id: string; polygon?: Point[]; rotation?: number; x?: number; y?: number;
    };
    ADD_IMPLANT: BaseOpParams & {
        type: Implant['type']; position: Point | null; angle: number;
        properties: Record<string, unknown>; fragmentId?: string | null;
    };
    UPDATE_IMPLANT: BaseOpParams & {
        id: string; position?: Point | null; angle?: number;
        properties?: Record<string, unknown>; fragmentId?: string | null;
    };
    DELETE_IMPLANT: BaseOpParams & { id: string };
    MOVE_IMPLANT: BaseOpParams & { id: string; deltaX: number; deltaY: number };
}

/** Union of every operation's parameters (the implementation-signature param type). */
export type AnyOperationParams = OperationParamsMap[OperationType];

export class CanvasManager {
    head: StateNode | null;
    current: StateNode | null;
    history: StateNode[];
    // Linear undo/redo model (MASTER §8.1 refinement): `history` holds past+present, `future`
    // holds the redo stack. This replaces the old redundant `redoStack` + `lastState` pair, whose
    // two slots always mirrored each other — one stack is the single source of truth for redo.
    private future: StateNode[];
    ...
    applyOperation<K extends OperationType>(operationType: K, params: OperationParamsMap[K]): Promise<StateNode>;
    async applyOperation(operationType: OperationType, params: AnyOperationParams): Promise<StateNode> {
        ...
        if (operationType === 'ROTATE') {
            const { fragmentId, angleDelta, center: customCenter } = params as OperationParamsMap['ROTATE'];
            ...
            const { startPoint, endPoint, fragmentId } = params as OperationParamsMap['CUT'];
            ...
            const { fragmentId } = params as OperationParamsMap['DELETE_FRAGMENT'];
            ...
            const { fragmentId, deltaX, deltaY } = params as OperationParamsMap['MOVE'];
            const fragment = newFragments.find(f => f.id === fragmentId);
            if (fragment) {
                fragment.x += deltaX;
                fragment.y += deltaY;
            ...
            const { points } = params as OperationParamsMap['FREEHAND_CUT'];
            ...
            const { toolKey, fragmentId, points, result, measurement } = params as OperationParamsMap['ADD_MEASUREMENT'];
            ...
            const { id, points, measurement: metadata, result: newResult } = params as OperationParamsMap['UPDATE_MEASUREMENT'];
            ...
            const { id } = params as OperationParamsMap['DELETE_MEASUREMENT'];
            const idx = newMeasurements.findIndex(m => m.id === id);
            if (idx !== -1) {
            ...
            const { points, type, id: updateId } = params as OperationParamsMap['WEDGE_OSTEOTOMY'];
            const [A, B, C] = points;
            ...
            const { oldFragmentId, newFragments: replacements } = params as OperationParamsMap['REPLACE_FRAGMENTS'];
            ...
            const { id, polygon, rotation, x, y } = params as OperationParamsMap['UPDATE_FRAGMENT'];
            ...
            const { type, position, angle, properties, fragmentId } = params as OperationParamsMap['ADD_IMPLANT'];
            ...
            const { id, position, angle, properties, fragmentId } = params as OperationParamsMap['UPDATE_IMPLANT'];
            ...
            const { id } = params as OperationParamsMap['DELETE_IMPLANT'];
            const idx = newImplants.findIndex(i => i.id === id);
            if (idx !== -1) {
            ...
            const { id, deltaX, deltaY } = params as OperationParamsMap['MOVE_IMPLANT'];
            const idx = newImplants.findIndex(i => i.id === id);
            if (idx !== -1) {
    ...
    undo() {
        // Pop the present off `history` (past+present) onto `future` (redo stack); the new top of
        // `history` becomes present. Linear model — one stack, no `lastState` shadow slot.
    ...
    redo() {
        // Pop the most recently undone state off `future` back onto `history` as the new present.
