/**
 * Cornerstone3D + tools init, custom local-metadata provider, tool groups, and file/URL loaders.
 *
 * PORTED from the old repo (src/renderer/lib/cornerstone/initCornerstone.ts) as part of Step 8b.
 * Behavior preserved EXCEPT one declared divergence (MASTER §8.3, see test/divergences.md): the
 * local-metadata key lookup now uses an EXACT normalized-key match instead of the old two-way
 * `endsWith` fuzzy match, which could attach the wrong slice's spacing/orientation when one imageId is
 * a suffix of another. Other edits are mechanical (unused `catch` bindings dropped for the strict
 * tsconfig); the `any` usage is intentionally retained under the ported-code eslint quarantine.
 */
...
// Verbose per-slice metadata logging is gated off by default (it fired on every metadata request —
// hundreds of lines per render and a measurable perf cost). Flip to true only when debugging metadata.
...
function localMetaDataProvider(type: string, imageId: string) {
    ...
    // DIVERGENCE (MASTER §8.3): exact normalized-key match only. The old code also accepted
    // `normImageId.endsWith(normKey) || normKey.endsWith(normImageId)`, which could match the wrong
    // entry when one imageId/filename is a suffix of another (e.g. slice_1.dcm vs slice_11.dcm),
    // attaching the wrong spacing/orientation to an image. See test/divergences.md.
    if (!dataset) {
        for (const [key, value] of localMetaDataMap.entries()) {
            const normKey = normalize(key);
            if (normImageId === normKey) {
                dataset = value;
                break;
            }
        }
    ...
    if (!dataset) {
        ...
        if (CS_DEBUG && (imageId.includes('wadouri') || imageId.includes('dicomfile') || imageId.includes('.dcm'))) {
    ...
    try {
        ...
        if (type === 'imagePixelModule') {
            ...
            if (CS_DEBUG) console.log(`[Cornerstone] Returning imagePixelModule for ${imageId}:`, result);
            return result;
        }
...
export function initCornerstone() {
    ...
    initPromise = (async () => {
        ...
        const workerFn = () => {
            // DIVERGENCE (8b): absolute '/decodeImageFrameWorker.js' instead of the old './…'. The old
            // app was always served at the site root, so './' resolved correctly; under the new SPA's
            // client-side routes (e.g. /dicom-verify) a relative URL resolves against the route path and
            // 404s. The asset lives at the public root, so an absolute path is correct everywhere.
